...
public class MyBehaviorTreeroom : MonoBehaviour
{
	public Transform light1;
	public Transform light2;
	public Transform light3;
	public Transform wander1forpolice1;
	public Transform wander2forpolice1;
    public Transform wander1forpolice2;
    public Transform wander2forpolice2;
	...
	void Start ()
	{
		...
		lightToSwitch1.enabled = true;
        lightToSwitch2.enabled = true;
        lightToSwitch3.enabled = true;
	...
	public void UpperLevel1() {
		Application.LoadLevel (1);
	}

	public void UpperLevel2() {
	...
	protected Node BuildTreeRoot() {
		...
		Node Daneilcontrol = new DecoratorLoop( //Daniel.GetComponent<BehaviorMecanim>().ST_PlayBodyGesture("Think", 1000),
                                         //   ST_LookAT(Daniel, light1),
                                         //   this.ST_ThinkUntilClicked(Daniel),
                                         //   this.ST_WalkTo (Daniel, light1),
                                         //  this.ST_turnonlight(Daniel, lightToSwitch1),
                                         //  ST_LookAT(Daniel, light2),
                                         //  this.ST_ThinkUntilClicked(Daniel),
                                         //  this.ST_WalkTo(Daniel, light2),
                                         //  this.ST_turnonlight(Daniel, lightToSwitch2)
                                       new DecoratorForceStatus (RunStatus.Success,
                                       new Sequence
                                       (   this.ST_ThinkUntilClicked(Daniel),
                                           this.IsClicked(Daniel)))
			             
                         );

        Node nodepolice1 = new DecoratorLoop(
                  new Sequence(this.ST_Repeatpolice(Police1, wander1forpolice1, wander2forpolice1)));
        Node nodepolice2 = new DecoratorLoop(
            new Sequence(this.ST_Repeatpolice(Police2, wander1forpolice2, wander2forpolice2)));

        /*
		Node nodeLuren = new Sequence (
			                 this.ST_ThinkUntilClicked (Luren1),
			                 this.Conversation (Luren1, Luren2, LurenMeetingPoint));
        ...
        Node root = new SequenceParallel (
			new Sequence (
				new SequenceParallel (
                    Daneilcontrol, nodepolice1, nodepolice2, gameover())));
	...
	public GameObject Luren1, Luren2;
	public Transform LurenMeetingPoint;
    ...
    */
    ...
    protected Node gameover()
    {
        ...
        return new DecoratorLoop(
            new SequenceParallel(
                new LeafAssert(act1), new LeafAssert(act2)));
    }
	...
	{
		if(guy.tag!="Police") return new LeafInvoke(()=>turnlight(guy));
    ...
    {
        return new Sequence(
             guy.GetComponent<BehaviorMecanim>().ST_PlayHandGesture("ReachRight", 1000), this.Turnlight(guy, light));
    }

    protected Node Turnlight(GameObject guy, Light light)
    {
...
*/
	...
	{
		Val<Vector3> position = Val.V (() => target.position);
		return new Sequence( guy.GetComponent<BehaviorMecanim>().Node_GoTo(position), new LeafWait(1000));
	}
	...
	protected Node IsClicked(GameObject guy) {
        ...
        return new SelectorParallel(
            new Sequence(
            new LeafInvoke(() => click1(guy)),
             this.ST_WalkTo(Daniel, light1),
             this.ST_turnonlight(Daniel, lightToSwitch1)),

            new Sequence(
            new LeafInvoke(() => click2(guy)),
             this.ST_WalkTo(Daniel, light2),
             this.ST_turnonlight(Daniel, lightToSwitch2)),

            new Sequence(
            new LeafInvoke(() => click3(guy)),
             this.ST_WalkTo(Daniel, light3),
             this.ST_turnonlight(Daniel, lightToSwitch3)));

    }

	RunStatus click1(GameObject guy) {
		Val<bool> click;
		if (guy.tag == "Daniel")
			click = Val.V (() => Input.GetKey(KeyCode.B));
		else
			return RunStatus.Failure;
		if (click.Value)
			return RunStatus.Success;
    ...
    RunStatus click2(GameObject guy)
    {
        Val<bool> click;
        if (guy.tag == "Daniel")
            click = Val.V(() => Input.GetKey(KeyCode.N));
        else
            return RunStatus.Failure;
        if (click.Value)
            return RunStatus.Success;
    ...
    RunStatus click3(GameObject guy)
    {
        Val<bool> click;
        if (guy.tag == "Daniel")
			click = Val.V(() => Input.GetKey(KeyCode.M));
        else
            return RunStatus.Failure;
        if (click.Value)
            return RunStatus.Success;
